...
Public Class ExceptionHandler
    ...
    Public Shared Sub Captura_Error(ByVal tipo As Exception)
        Try
            ExceptionHandler.MailException(tipo)
    ...
    '        Next
    '        If Not HttpContext.Current.Session("user_session") Is Nothing Then mailMessage.Subject = "Mensaje Hunter Cobros"
    '        If Not HttpContext.Current.Session("user") Is Nothing Then mailMessage.Subject = "Mensaje Hunter Cobros"
    '        mailMessage.Body = MailBody(exception)
    '        mailMessage.Priority = MailPriority.High
    '        Dim smtpClient As New SmtpClient("10.100.89.4")
    ...
    Private Shared Sub MailException(ByVal exception As Exception)
        Try
            Dim mailMessage As New MailMessage()
            ...
            For Each mailTo As String In mailToCollection
                mailMessage.To.Add(mailTo)
            Next
            mailMessage.Subject = "Error en " + ConfigurationManager.AppSettings.Get("ProyectName").ToString()
            mailMessage.Body = MailBody(exception)
            mailMessage.Priority = MailPriority.High
            'Dim smtpClient As New SmtpClient(ConfigurationManager.AppSettings.Get("SmptCliente").ToString())
            'smtpClient.Port = ConfigurationManager.AppSettings.Get("SmptPort").ToString()
            'smtpClient.Send(mailMessage)
    ...
    Private Shared Function MailBody(ByVal exception As Exception) As String
        ...
        Try
            ...
            If Not HttpContext.Current.Session("user_session") Is Nothing Then
                ...
                titulo = "Hunter Cobros"
                'ipMaquina = HttpContext.Current.Session("user_ip").ToString()
            End If
            If Not HttpContext.Current.Session("user") Is Nothing Then
                usuario = HttpContext.Current.Session("user").ToString()
                titulo = "Hunter Cobros"
                'ipMaquina = HttpContext.Current.Session("usuario_ip").ToString()
            End If
            ...
            'builder.Append("Error en Sistema : '").Append("Sección Gerencial").Append("'</div></tr>")
            builder.Append("Error en Sistema : '").Append(titulo).Append("'</div></tr>")
